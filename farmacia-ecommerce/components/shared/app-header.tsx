...
export function AppHeader() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 flex h-14 flex-shrink-0 items-center gap-3 bg-brand-500 px-4 text-white shadow-[0_2px_8px_rgba(0,0,0,.2)]">
        <button
        ...
        <div className="ml-auto flex flex-shrink-0 items-center gap-2">
          <button onClick={() => setCartOpen(true)} className="relative flex h-9 w-9 items-center justify-center rounded-lg bg-white/15">
            <ShoppingCart className="h-4 w-4" />
            {cartCount > 0 && (
              <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-error-500 text-[9px] font-black">
                {cartCount}
              </span>
            )}
          </button>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-50 flex bg-black/50" onClick={() => setMenuOpen(false)}>
          <div className="flex h-full w-[82vw] max-w-[280px] animate-slide-in flex-col bg-white" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between bg-brand-500 p-4 text-white">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <div className="text-[10px] uppercase tracking-wide text-white/70">Portal do Cliente</div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-auto p-2.5">
              {LINKS.map((l) => (
                <Link
                  ...
                  className={cn(
                    ...
                    pathname === l.href ? 'bg-brand-50 text-brand-600' : 'text-navy-700'
                  )}
                >
              ...
              <button
                ...
                className="flex min-h-[44px] w-full items-center gap-2.5 rounded-lg px-3.5 py-3 text-left text-[14px] font-semibold text-navy-700"
              >
                <span className="text-lg">🛒</span> Carrinho {cartCount > 0 && `(${cartCount})`}
              </button>
            </div>
